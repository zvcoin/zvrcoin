...
#if defined(NDEBUG)
# error "Zvrcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x098a4d6ce739cfe9ee40ef3458131f1621ea572f6c28c3488555cb28e3b16c6b");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zvrcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Zvrcoin Signed Message:\n";
...
{
    // Zvrcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Zvrcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Zvrcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Zvrcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zvrcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zvrcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x098a4d6ce739cfe9ee40ef3458131f1621ea572f6c28c3488555cb28e3b16c6b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Welcome to ZvrCoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1508828352;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1377031;
        ...
        assert(block.hashMerkleRoot == uint256("0x8c7b981510e38d4d53f7c34eb14f6d65e895262a2de990debf5caf23592c72bb"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Zvrcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZvrcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZvrcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZvrcoinMiner(CWallet *pwallet)
{
    printf("ZvrcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zvrcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ZvrcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZvrcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZvrcoinMiner, pwallet));
}
